...
- **Marketing “create from video”** — `/studio/create` can run **free** server-side analysis: OpenCV optical-flow segmentation (`scripts/segment_optical_flow.py`, requires `opencv-python-headless`), optional **Whisper** CLI (`pip install openai-whisper`, `whisper` on `PATH`) for local ASR, optional **Tesseract** for OCR. If those are missing, the pipeline falls back to FFmpeg scene heuristics and leaves ASR/OCR hints empty. **Workflow generation and analysis are not billed to the user**; **fal** generative calls and fal storage still use the user’s API key (or `FAL_KEY`) like the rest of Studio.
